...
async function $and(args, scope) {
    ...
    while (stilltrue && i < args.length) {
        if (util.isfalsy (await ea.eval_sexpr(args[i]), scope))
            stilltrue = false;
        i++;
    }
    return stilltrue;
...
async function $or(args, scope) {
    ...
    while (stillfalse && i < args.length) {
        if (util.istruthy (await ea.eval_sexpr(args[i]), scope))
            stillfalse = false;
        i++;
    }
    return !stillfalse;
...
async function $not(args, scope) {
    return !util.istruthy(await ea.eval_sexpr(args[0], scope));
}
