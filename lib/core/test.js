...
async function assert(str, exp) {
    if (exp == null)
        exp = true;
    var act = await ea.eval_str(str);
    asserttest(exp, act, str);
}


async function prep(str) {
    await ea.eval_str(str);
}


async function sform$fassert(args, scope) {
    var exp = await ea.eval_sexpr(args[0], scope),
        act = await ea.eval_sexpr(args[1], scope);
    asserttest(exp, act, args[0]);
}


async function test_main(args, scope) {
    test.asserted = 0;
    test.failed = 0;
