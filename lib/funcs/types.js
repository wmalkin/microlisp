...
function $tolisp(args, scope) {
    return util.tolisp(args[0]);
}


function $tojson(args, scope) {
    return util.tojson(args[0]);
}
...
func.def({ name: "tolisp", body: $tolisp });
func.def({ name: "tojson", body: $tojson });
