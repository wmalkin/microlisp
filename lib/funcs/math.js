...
function $add(args, scope) {
    ...
    function add(a, b) {
        if (util.isdate(a))
            return new Date(a.getTime() + util.tonum(b));
        else if (util.isdate(b))
            return new Date(b.getTime() + util.tonum(a));
        else
            return util.tonum(a) + util.tonum(b);
    }
...
function $mul(args, scope) {
    var result = null;
    if (args.length) {
        result = util.tonum(args[0]);
        for (var i = 1; i < args.length; i++)
            result *= util.tonum(args[i]);
    }
    return result;
}
...
function $min(args, scope) {
    if (args.length == 1 && util.islist(args[0]))
        args = args[0];
    var m = args[0];
    for (var i = 1; i < args.length; i++)
        if (args[i] < m)
...
func.def({ name: "min", body: $min });
func.def({ name: "max", body: $max });
