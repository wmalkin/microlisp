...
async function $for(args, scope) {
    var rs = null,
            ...
            gsym = options[0],
            ifrom = 0,
            ito = util.tonum(await ea.eval_sexpr(options[1], scope)),
            iinc = 1;
    if (options.length > 2) {
        ifrom = ito;
        ito = util.tonum(await ea.eval_sexpr(options[2], scope));
    }
    if (options.length > 3)
        iinc = util.tonum(await ea.eval_sexpr(options[3], scope));
